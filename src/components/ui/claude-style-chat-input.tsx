...
const FilePreviewCard: React.FC<{ file: AttachedFile; onRemove: (id: string) => void }> = ({ file, onRemove }) => {
  ...
  return (
    <div className="relative group flex-shrink-0 w-24 h-24 rounded-xl overflow-hidden border border-border bg-muted transition-all hover:border-foreground/40">
      {isImage ? (
        ...
        <div className="w-full h-full p-3 flex flex-col justify-between">
          ...
          <div className="space-y-0.5">
            <p className="text-xs font-medium text-foreground truncate" title={file.file.name}>{file.file.name}</p>
...
const PastedContentCard: React.FC<{ content: PastedContent; onRemove: (id: string) => void }> = ({ content, onRemove }) => (
  <div className="relative group flex-shrink-0 w-28 h-28 rounded-2xl overflow-hidden border border-border bg-background p-3 flex flex-col justify-between shadow-sm">
    ...
    <div className="inline-flex items-center justify-center px-1.5 py-[2px] rounded border border-border self-start">
...
const ModelSelector: React.FC<{ models: Model[]; selectedModel: string; onSelect: (id: string) => void }> = ({
  models,
  selectedModel,
  onSelect,
}) => {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className={cn(
          "inline-flex items-center justify-center gap-1 h-8 rounded-xl px-2.5 text-xs font-medium transition-colors active:scale-[0.98]",
          isOpen ? "bg-muted text-foreground" : "text-muted-foreground hover:text-foreground hover:bg-muted"
        )}
      >
        <span className="whitespace-nowrap select-none">{currentModel.name}</span>
        <Icons.SelectArrow className={cn("h-4 w-4 opacity-75 transition-transform duration-200", isOpen && "rotate-180")} />
      </button>

      {isOpen && (
        <div className="absolute bottom-full right-0 mb-2 w-[260px] bg-popover border border-border rounded-2xl shadow-2xl overflow-hidden z-50 flex flex-col p-1.5 origin-bottom-right animate-in fade-in-0 zoom-in-95">
          {models.map((model) => (
            <button
              key={model.id}
              onClick={() => { onSelect(model.id); setIsOpen(false); }}
              className="w-full text-left px-3 py-2.5 rounded-xl flex items-start justify-between transition-colors hover:bg-muted"
            >
              ...
              {selectedModel === model.id && <Icons.Check className="w-4 h-4 text-purple-600 mt-1" />}
            </button>
          ))}
        </div>
      )}
...
/* --- MAIN CHAT INPUT --- */
export interface ClaudeChatSendData {
  message: string;
  files: AttachedFile[];
...
interface ClaudeChatInputProps {
  onSendMessage: (data: ClaudeChatSendData) => void;
  placeholder?: string;
  footer?: string;
}
...
export const ClaudeChatInput: React.FC<ClaudeChatInputProps> = ({
  onSendMessage,
  placeholder = "Descreva o e-mail que a IA deve criar...",
  footer = "A IA pode cometer erros. Verifique informações importantes.",
}) => {
  ...
  const models: Model[] = [
    { id: "opus-4.5", name: "Opus 4.5", description: "Mais capaz para trabalhos complexos" },
    { id: "sonnet-4.5", name: "Sonnet 4.5", description: "Ótimo para o dia a dia" },
    { id: "haiku-4.5", name: "Haiku 4.5", description: "Mais rápido para respostas rápidas" },
  ];
  ...
  const handlePaste = (e: React.ClipboardEvent) => {
    ...
    if (text.length > 300) {
      e.preventDefault();
      setPastedContent((prev) => [...prev, { id: crypto.randomUUID(), content: text, timestamp: new Date() }]);
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        {/* Solid border first, colored one just below — snug to the bottom edge, tapered/faded at the ends */}
        <div className="pointer-events-none absolute left-6 right-6 bottom-[1px] h-[2px] rounded-full bg-zinc-300 dark:bg-zinc-600" style={{ WebkitMaskImage: "linear-gradient(90deg, transparent, #000 18%, #000 82%, transparent)", maskImage: "linear-gradient(90deg, transparent, #000 18%, #000 82%, transparent)" }} />
        <div className="chat-anim-border pointer-events-none absolute left-10 right-10 -bottom-[3px] h-[2px] rounded-full" style={{ WebkitMaskImage: "linear-gradient(90deg, transparent, #000 18%, #000 82%, transparent)", maskImage: "linear-gradient(90deg, transparent, #000 18%, #000 82%, transparent)" }} />
        <div className="flex flex-col px-3 pt-3 pb-2 gap-2">
          ...
          <div className="relative mb-1">
            <div className="max-h-96 w-full overflow-y-auto break-words min-h-[2.5rem] pl-1">
              <textarea
                ref={textareaRef}
                value={message}
                ...
                className="w-full bg-transparent border-0 outline-none text-foreground text-[15px] placeholder:text-muted-foreground resize-none overflow-hidden py-0 leading-relaxed block"
                rows={1}
              />
            </div>
          </div>
          ...
          <div className="flex gap-2 w-full items-center">
            <div className="relative flex-1 flex items-center shrink min-w-0 gap-1">
              <button
                ...
                className="inline-flex items-center justify-center shrink-0 transition-colors h-8 w-8 rounded-lg active:scale-95 text-muted-foreground hover:text-foreground hover:bg-muted"
                type="button"
                aria-label="Anexar"
              >
              ...
              <button
                ...
                className={cn(
                  ...
                  isThinkingEnabled ? "text-purple-700 bg-purple-100" : "text-muted-foreground hover:text-foreground hover:bg-muted"
                )}
                aria-pressed={isThinkingEnabled}
                aria-label="Raciocínio estendido"
                type="button"
              >
            ...
            <div className="flex flex-row items-center min-w-0 gap-1">
              ...
              <button
                ...
                disabled={!hasContent}
                className={cn(
                  "inline-flex items-center justify-center shrink-0 transition-colors h-8 w-8 rounded-xl active:scale-95",
                  hasContent ? "bg-black text-white hover:bg-black/90" : "bg-muted text-muted-foreground cursor-default"
                )}
                type="button"
                aria-label="Enviar"
              >
      ...
      {isDragging && (
        <div className="absolute inset-0 bg-background/90 border-2 border-dashed border-purple-500 rounded-2xl z-50 flex flex-col items-center justify-center backdrop-blur-sm pointer-events-none">
          <p className="text-purple-600 font-medium">Solte os arquivos aqui</p>
        </div>
      )}
