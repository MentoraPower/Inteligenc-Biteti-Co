...
serve(async (req) => {
  ...
  try {
    ...
    const formId = String(url.searchParams.get("form_id") || body?.form_id || "").trim();
    ...
    // Lookup by id AND by label so the mapping can reference either.
    ...
    // Also pass raw fields (by id and label) so receive-webhook aliases/labels catch extras.
    for (const f of rawFields) {
    ...
    // Tag + source markers for the lead history.
